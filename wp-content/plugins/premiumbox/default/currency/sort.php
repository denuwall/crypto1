...
global $wpdb;
	...
	$datas = $wpdb->get_results("SELECT * FROM ".$wpdb->prefix."currency WHERE auto_status = '1' ORDER BY site_order ASC");
	$sort_list = array();
	...
	if(current_user_can('administrator') or current_user_can('pn_currency')){
		...
		if(is_array($number)){	
			foreach($number as $theid) { $y++;
				$theid = intval($theid);
				$wpdb->query("UPDATE ".$wpdb->prefix."currency SET site_order='$y' WHERE id = '$theid'");	
			}	
		}
	}
}