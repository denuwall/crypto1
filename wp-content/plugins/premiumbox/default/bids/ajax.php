				...
				foreach($_POST['id'] as $id){
					...
					if(isset($item->id)){
						...
						if($result == 1){
							...
							do_action('change_bidstatus_all', 'realdelete', $item->id, $item, 'admin_panel', 'user');
							do_action('change_bidstatus_realdelete', $item->id, $item, 'admin_panel', 'user'); 						
			...
			if(is_array($bid_status_list)){
				...
				if(in_array($action, $sts)){
					foreach($_POST['id'] as $id){
						...
						if(isset($item->id)){
							$result = $wpdb->update($wpdb->prefix.'exchange_bids', array('status'=>$action, 'edit_date'=> $time), array('id'=>$id));
							if($result == 1){
								do_action('change_bidstatus_all', $action, $item->id, $item, 'admin_panel', 'user');
								do_action('change_bidstatus_'.$action, $item->id, $item, 'admin_panel', 'user');							
							}
						}
					}
				}	
...
global $wpdb;
	...
	/*********/
	$currencies = apply_filters('list_currency_manage', array(), __('All','pn'));
	...
	/*********/
	$vtype = $wpdb->get_results("SELECT * FROM ". $wpdb->prefix ."currency_codes WHERE auto_status='1' ORDER BY currency_code_title ASC");
	$options = array(
		'0' => '--'. __('All','pn').'--',
	);
	foreach($vtype as $item){ 
		$options[$item->id] = is_site_value($item->currency_code_title);
	}
...
global $wpdb, $premiumbox;	
	...
	if(current_user_can('administrator') or current_user_can('pn_bids')){
		...
		if($paystatus == 1){
			$where = " AND {$pr}exchange_bids.status IN('new','cancel','delete','techpay','error','payouterror','my','coldpay','coldsuccess','success')";
		} elseif($paystatus == 2){
			$where = " AND {$pr}exchange_bids.status IN('payed','realpay','verify')";
		}
		
		$bidstatus = is_isset($pars_data,'bidstatus');
		if(is_array($bidstatus)){ 
			$where = " AND {$pr}exchange_bids.status = '1'";
			if(count($bidstatus) > 0){
				...
				if(count($in_bs) > 0){
					...
					$where = " AND {$pr}exchange_bids.status IN($in_bs_join)";
				}
			}
		} else {
			$bidstatus = is_status_name($bidstatus);
			if($bidstatus){
				$where = " AND {$pr}exchange_bids.status = '$bidstatus'";
			}
		}
		...
		if($startdate){
			...
			$where .= " AND {$pr}exchange_bids.create_date >= '$startdate'";
		}
		$enddate = is_my_date(is_isset($pars_data,'enddate'));
		if($enddate){
			$enddate = get_mydate($enddate,'Y-m-d 00:00');
			$where .= " AND {$pr}exchange_bids.create_date <= '$enddate'";
		}
		$min_sum1 = is_sum(is_isset($pars_data,'min_sum1'));
		if($min_sum1 > 0){
			$where .= " AND {$pr}exchange_bids.sum1dc >= ('$min_sum1' -0.0)";
		}
		$min_sum2 = is_sum(is_isset($pars_data,'min_sum2'));
		if($min_sum2 > 0){
			$where .= " AND {$pr}exchange_bids.sum2c >= ('$min_sum2' -0.0)";
		}
		$direction_id = intval(is_isset($pars_data,'direction_id'));
		if($direction_id > 0){
			$where .= " AND {$pr}exchange_bids.direction_id = '$direction_id'";
		}		
		$v1 = intval(is_isset($pars_data,'v1'));
		if($v1 > 0){
			$where .= " AND {$pr}exchange_bids.currency_id_give = '$v1'";
		}
		$v2 = intval(is_isset($pars_data,'v2'));
		if($v2 > 0){
			$where .= " AND {$pr}exchange_bids.currency_id_get = '$v2'";
		}
		$psys1 = intval(is_isset($pars_data,'psys1'));
		if($psys1 > 0){
			$where .= " AND {$pr}exchange_bids.psys_id_give = '$psys1'";
		}
		$psys2 = intval(is_isset($pars_data,'psys2'));
		if($psys2 > 0){
			$where .= " AND {$pr}exchange_bids.psys_id_get = '$psys2'";
		}
		$vtype1 = intval(is_isset($pars_data,'vtype1'));
		if($vtype1 > 0){
			$where .= " AND {$pr}exchange_bids.currency_code_id_give = '$vtype1'";
		}		
		$vtype2 = intval(is_isset($pars_data,'vtype2'));
		if($vtype2 > 0){
			$where .= " AND {$pr}exchange_bids.currency_code_id_get = '$vtype2'";
		}
		
		$iduser = intval(is_isset($pars_data,'iduser'));
		if($iduser > 0){
			$where .= " AND {$pr}exchange_bids.user_id='$iduser'";
		} else {	
			$user_login = is_user(is_isset($pars_data,'user_login'));
			if($user_login){
				$user_id = username_exists($user_login);
				if($user_id){
					$where .= " AND {$pr}exchange_bids.user_id='$user_id'";
				}
			}
		}
		...
		if($mini_navi == 1){
			...
			$count_bids = $wpdb->get_var("SELECT COUNT({$pr}exchange_bids.id) FROM {$pr}exchange_bids WHERE 1 $where");
			$pagenavi = get_pagenavi_calc($limit,$paged,$count_bids);
		}
			
		//$sql = "SELECT *, {$pr}exchange_bids.id AS bid_id FROM {$pr}exchange_bids WHERE status != 'auto' $where ORDER BY create_date DESC LIMIT {$pagenavi['offset']}, {$pagenavi['limit']}";
		$sql = "SELECT *, {$pr}exchange_bids.id AS bid_id FROM {$pr}exchange_bids WHERE 1 $where ORDER BY create_date DESC LIMIT {$pagenavi['offset']}, {$pagenavi['limit']}";
			...
			';
				...
				<div class="bids_col">';
					...
					$cols['rate'] = array(
						'type' => 'text',
						'title' => __('Rate','pn'),
						'label' => '[course_give] [vtype_give] = [course_get] [vtype_get]',
					...
					if($item->edit_date != '0000-00-00 00:00:00'){
						$cols['editdate'] = array(
							'type' => 'text',
				...
				<div class="bids_col">';
					...
					$cols['sum1dc'] = array(
						...
						'label' => '[sum1dc] [vtype_give]',
						'class' => 'btbg_green',
					);
					$cols['sum1c'] = array(
						'type' => 'text',
						'title' => __('Amount (with add. fees and PS fees)','pn'),
						'label' => '[sum1c] [vtype_give]',
					);	
				...
				<div class="bids_col">';
					...
					$cols['sum2dc'] = array(
						'type' => 'text',
						'title' => __('Amount (with add. fees)','pn'),
						'label' => '[sum2dc] [vtype_get]',
						'class' => 'btbg_green',
					);
					$cols['sum2c'] = array(
						'type' => 'text',
						'title' => __('Amount (with add. fees and PS fees)','pn'),
						'label' => '[sum2c] [vtype_get]',
					);
				...
				';
					...
					<div class="bi_block">';
						...
						$cols['sum2t'] = array(
							...
							'label' => '[sum2t] [vtype_get]',
						);
						$cols['sum2'] = array(
							'type' => 'text',
							'title' => __('Amount (discount included)','pn'),
							'label' => '[sum2] [vtype_get]',
						);	
						$cols['dop_com2'] = array(
							'type' => 'text',
							'title' => __('Add. fees amount','pn'),
...
function get_bids_replace_text($text, $item, $data_fs, $v){
	...
	if(strstr($text, '[user_discount]')){
		$text = str_replace('[user_discount]', '<span class="onebid_item item_user_discount clpb_item" data-clipboard-text="' . pn_strip_input($item->user_discount) . '">' . check_podmena_db('user_discount', $item, $data_fs) . '</span>',$text);
	}	
	if(strstr($text, '[user_discount_sum]')){
		$text = str_replace('[user_discount_sum]', '<span class="onebid_item item_user_discount_sum clpb_item" data-clipboard-text="' . is_sum($item->user_discount_sum) . '">' . check_podmena_db('user_discount_sum', $item, $data_fs) . '</span>',$text);
	}
	...
	if(strstr($text, '[vtype_give]')){
		$text = str_replace('[vtype_give]', '<span class="onebid_item item_vtype_give clpb_item" data-clipboard-text="' . pn_strip_input($item->currency_code_give) . '">' . pn_strip_input($item->currency_code_give) . '</span>',$text);
	}
	if(strstr($text, '[vtype_get]')){
		$text = str_replace('[vtype_get]', '<span class="onebid_item item_vtype_get clpb_item" data-clipboard-text="' . pn_strip_input($item->currency_code_get) . '">' . pn_strip_input($item->currency_code_get) . '</span>',$text);
	}	
	if(strstr($text, '[sum1]')){
		$text = str_replace('[sum1]', '<span class="onebid_item item_summ1 clpb_item" data-clipboard-text="' . pn_strip_input($item->sum1) . '">' . check_podmena_db('sum1', $item, $data_fs) . '</span>',$text);
	}	
	...
	if(strstr($text, '[sum1dc]')){
		$text = str_replace('[sum1dc]', '<span class="onebid_item item_summ1_dc clpb_item" data-clipboard-text="' . pn_strip_input($item->sum1dc) . '">' . check_podmena_db('sum1dc', $item, $data_fs) . '</span>',$text);
	}
	...
	if(strstr($text, '[sum1c]')){
		$text = str_replace('[sum1c]', '<span class="onebid_item item_summ1c clpb_item" data-clipboard-text="' . pn_strip_input($item->sum1c) . '">' . check_podmena_db('sum1c', $item, $data_fs) . '</span>',$text);
	}
	if(strstr($text, '[sum1r]')){
		$text = str_replace('[sum1r]', '<span class="onebid_item item_summ1cr clpb_item" data-clipboard-text="' . pn_strip_input($item->sum1r) . '">' . check_podmena_db('sum1r', $item, $data_fs) . '</span>',$text);
	}
	if(strstr($text, '[sum2t]')){
		$text = str_replace('[sum2t]', '<span class="onebid_item item_summ2t clpb_item" data-clipboard-text="' . pn_strip_input($item->sum2t) . '">' . check_podmena_db('sum2t', $item, $data_fs) . '</span>',$text);
	}	
	if(strstr($text, '[sum2]')){
		$text = str_replace('[sum2]', '<span class="onebid_item item_summ2 clpb_item" data-clipboard-text="' . pn_strip_input($item->sum2) . '">' . check_podmena_db('sum2', $item, $data_fs) . '</span>',$text);
	}
	...
	if(strstr($text, '[sum2dc]')){
		$text = str_replace('[sum2dc]', '<span class="onebid_item item_summ2_dc clpb_item" data-clipboard-text="' . pn_strip_input($item->sum2dc) . '">' . check_podmena_db('sum2dc', $item, $data_fs) . '</span>',$text);
	}	
	...
	if(strstr($text, '[sum2c]')){
		$text = str_replace('[sum2c]', '<span class="onebid_item item_summ2c clpb_item" data-clipboard-text="' . pn_strip_input($item->sum2c) . '">' . check_podmena_db('sum2c', $item, $data_fs) . '</span>',$text);
	}	
	if(strstr($text, '[sum2r]')){
		$text = str_replace('[sum2r]', '<span class="onebid_item item_summ2cr clpb_item" data-clipboard-text="' . pn_strip_input($item->sum2r) . '">' . check_podmena_db('sum2r', $item, $data_fs) . '</span>',$text);
	}	
	...
	if(strstr($text, '[account_give]')){
		$text = str_replace('[account_give]', '<span class="onebid_item item_account_give clpb_item" data-clipboard-text="' . pn_strip_input($item->account_give) . '">' . check_podmena_db('account_give', $item, $data_fs, $v) . '</span>',$text);
	}
	if(strstr($text, '[account_get]')){
		$text = str_replace('[account_get]', '<span class="onebid_item item_account_get clpb_item" data-clipboard-text="' . pn_strip_input($item->account_get) . '">' . check_podmena_db('account_get', $item, $data_fs, $v) . '</span>',$text);
	}	
	...
	if(strstr($text, '[course_give]')){
		$text = str_replace('[course_give]', '<span class="onebid_item item_curs1 clpb_item" data-clipboard-text="' . pn_strip_input($item->course_give) . '">' . pn_strip_input($item->course_give) . '</span>',$text);
	}	
	if(strstr($text, '[course_get]')){
		$text = str_replace('[course_get]', '<span class="onebid_item item_curs2 clpb_item" data-clipboard-text="' . pn_strip_input($item->course_get) . '">' . pn_strip_input($item->course_get) . '</span>',$text);
	}
	if(strstr($text, '[direction_id]')){
		$text = str_replace('[direction_id]', '<span class="onebid_item item_naps_id clpb_item" data-clipboard-text="' . pn_strip_input($item->direction_id) . '">' . pn_strip_input($item->direction_id) . '</span>',$text);
	}	
	if(strstr($text, '[createdate]')){
		$text = str_replace('[createdate]', '<span class="onebid_item item_createdate clpb_item" data-clipboard-text="' . get_mytime($item->create_date,'d.m.Y H:i:s') . '">' . get_mytime($item->create_date,'d.m.Y H:i:s') . '</span>',$text);
	}
	if(strstr($text, '[editdate]')){
		$text = str_replace('[editdate]', '<span class="onebid_item item_editdate clpb_item" data-clipboard-text="' . get_mytime($item->edit_date,'d.m.Y H:i:s') . '">' . get_mytime($item->edit_date,'d.m.Y H:i:s') . '</span>',$text);
	}	
	if(strstr($text, '[from_account]')){
		$text = str_replace('[from_account]', '<span class="onebid_item item_soschet clpb_item" data-clipboard-text="' . pn_strip_input($item->from_account) . '">' . check_podmena_db('from_account', $item, $data_fs) . '</span>',$text);
	}
	if(strstr($text, '[to_account]')){
		$text = str_replace('[to_account]', '<span class="onebid_item item_naschet clpb_item" data-clipboard-text="' . pn_strip_input($item->to_account) . '">' . check_podmena_db('to_account', $item, $data_fs) . '</span>',$text);
	}
	...
	if(strstr($text, '[currency_give]')){
		$text = str_replace('[currency_give]', '<span class="onebid_item item_currency_give clpb_item" data-clipboard-text="' . pn_strip_input(ctv_ml($item->psys_give)) . '">' . pn_strip_input(ctv_ml($item->psys_give)) . '</span>',$text);
	}
	if(strstr($text, '[currency_get]')){
		$text = str_replace('[currency_get]', '<span class="onebid_item item_currency_get clpb_item" data-clipboard-text="' . pn_strip_input(ctv_ml($item->psys_get)) . '">' . pn_strip_input(ctv_ml($item->psys_get)) . '</span>',$text);
	}	
	if(strstr($text, '[account_give_visible]')){
		$text = str_replace('[account_give_visible]', '<span class="onebid_item item_account_give_visible clpb_item" data-clipboard-text="'. pn_strip_input($item->account_give) .'">' . check_podmena_db('account_give', $item, $data_fs, $v) . '</span>',$text);
	}
	if(strstr($text, '[account_get_visible]')){
		$text = str_replace('[account_get_visible]', '<span class="onebid_item item_account_get_visible clpb_item" data-clipboard-text="'. pn_strip_input($item->account_get) .'">' . check_podmena_db('account_get', $item, $data_fs, $v) . '</span>',$text);
	}	
