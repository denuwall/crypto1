...
class trev_wchecks_List_Table extends WP_List_Table { 
    ...
    function column_default($item, $column_name){
		...
		if($column_name == 'descr'){
			...
			return '<a href="'. admin_url('admin.php?page=pn_wchecks&place='. is_isset($item, 'place')) .'&mod='. is_param_get('mod') .'">'. $place . '</a>';			
    ...
    function column_title($item){
		...
		if($item['status'] == 'active'){
			$actions['deactive']  = '<a href="'. pn_link_post('pn_wchecks_deactivate'). '&key='. $item['name'] .'&_wp_http_referer=' . urlencode($_SERVER['REQUEST_URI']) .'">'. __('Deactivated','pn') .'</a>';
		} else {
			$actions['active']  = '<a href="'. pn_link_post('pn_wchecks_activate'). '&key='. $item['name'] .'&_wp_http_referer=' . urlencode($_SERVER['REQUEST_URI']) .'">'. __('Activated','pn') .'</a>';
		}
