	...
	if(isset($_POST['id']) and is_array($_POST['id'])){
		...
		if($action == 'active'){
			...
			foreach($_POST['id'] as $id){
				...
				if($id){
					if(!isset($extended['paymerchants'][$id])){
						$extended['paymerchants'][$id] = $id;
						
						pn_include_extanded('paymerchants', $id);
						
						do_action('pn_paymerchants_active_'.$id);
						do_action('pn_paymerchants_active', $id);
					}
				}	
			}
		...
		if($action == 'deactive'){
			...
			foreach($_POST['id'] as $id){
				...
				if($id){
					if(isset($extended['paymerchants'][$id])){
						unset($extended['paymerchants'][$id]);
						
						do_action('pn_paymerchants_deactive_'.$id);
						do_action('pn_paymerchants_deactive', $id);
...
class trev_paymerchants_List_Table extends WP_List_Table { 
    ...
    function column_default($item, $column_name){
		...
		if($column_name == 'descr'){
			...
			return '<a href="'. admin_url('admin.php?page=pn_paymerchants&place='. is_isset($item, 'place')) .'&mod='. is_param_get('mod') .'">'. $place . '</a>';			
