...
add_action('pn_direction_delete', 'pn_direction_delete_reservcurs');
function pn_direction_delete_reservcurs($item_id){ 
global $wpdb;	
	$wpdb->query("DELETE FROM ".$wpdb->prefix."naps_reservcurs WHERE naps_id = '$item_id'");
}

add_action('pn_direction_copy', 'pn_direction_copy_reservcurs', 1, 2);  
function pn_direction_copy_reservcurs($last_id, $new_id){
global $wpdb;
...
add_action('tab_direction_tab2', 'tab_direction_tab2_reservcurs');
...
add_action('after_update_currency_reserv', 'reservcurs_after_update_currency_reserv', 100, 3);
function reservcurs_after_update_currency_reserv($currency_reserv, $currency_id, $item){
global $wpdb, $premiumbox; 
		
	$what = intval($premiumbox->get_option('reservcurs','what'));
	$directions = $wpdb->get_results("SELECT * FROM ". $wpdb->prefix ."directions WHERE currency_id_get = '$currency_id'");
	foreach($directions as $direction){
		$direction_id = $direction->id;
		$course_give = $direction->course_give;
		$course_get = $direction->course_get;
		...
		if(isset($data->id)){
			...
			if($n_course_give > 0 and $n_course_get > 0){
				...
				do_action('direction_change_course', $direction_id, $direction, $n_course_give, $n_course_get, 'reservcurs');
			}
		}		
	}
}	
