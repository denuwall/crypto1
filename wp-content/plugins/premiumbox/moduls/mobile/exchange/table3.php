	...
	if(!isset($direction->id)){
		...
		if(isset($direction->id)){
			...
			if($premiumbox->get_option('exchange','flysum') == 1){
				...
				if($sum1 < $min1){
					$sum1_error = 'error';
					$sum1_error_text = __('min','pn').'.: <span class="js_amount">'. $min1 .'</span> '. $currency_code_give;													
				}
										
				if($sum1 > $max1 and is_numeric($max1)){
					$sum1_error = 'error';
					$sum1_error_text = __('max','pn').'.: <span class="js_amount">'. $max1 .'</span> '. $currency_code_give;													
				}
										
				if($sum2 < $min2){
					$sum2_error = 'error';
					$sum2_error_text = __('min','pn').'.: <span class="js_amount">'. $min2 .'</span> '. $currency_code_get;													
				}
										
				if($sum2 > $max2 and is_numeric($max2)){
					$sum2_error = 'error';
					$sum2_error_text = __('max','pn').'.: <span class="js_amount">'. $max2 .'</span> '. $currency_code_get;													
				}			
			}
		...
		if(isset($direction->id)){			 			
			...
			$temp .= '<input type="hidden" name="" class="js_direction_id" value="'. $direction->id .'" />';
			
			$temp .='	
			<div class="xtl_submit_wrap">
				<div class="xtl_submit_ins">	
					<a href="'. get_exchange_link($direction->direction_name) .'" class="xtl_submit js_exchange_link" id="js_submit_button" data-direction-id="'. $direction->id .'">'. __('Exchange','pn') .'</a>
						<div class="clear"></div>
				</div>
			</div>';	
			...
			$tbl3_rightcol_data = array(
				...
				<div class="xtl_line xtl_exchange_reserve">
					'. __('Reserve','pn') .': <span>'. $reserv .' '. $currency_code_get .'</span>
				</div>						
				',
			);
