        ...
        <tr>
			...
			<div class="premium_wrap_standart">
				...
				$array = array(
					...
					'com_sum1_check' => __('Fee Send for verfified account','pn'),
					'com_pers1_check' => __('Fee (%) Send for verfified account','pn'),
					'com_sum2_check' => __('Fee Receive for verfified account','pn'),
...
global $wpdb;	
	...
	if(is_array($data)){
		...
		$export_filter = array(
			'int_arr' => array('id'),
			'qw_arr' => array('direction_status', 'enable_user_discount'),
			'sum_arr' => array(
				'currency_reserv','course_give','course_get','max_user_discount',
												...
												if(isset($bd_array['psys_id_give']) and isset($bd_array['currency_code_id_give'])){
													if($bd_array['psys_id_give'] and $bd_array['currency_code_id_give']){
														...
														if(isset($vals->id)){
															...
															$uniq = pn_stryp_symbols(replace_cyr($bd_array['currency_give']));
											...
											if(isset($bd_array['psys_id_get']) and isset($bd_array['currency_code_id_get'])){
												if($bd_array['psys_id_get'] and $bd_array['currency_code_id_get']){
													...
													if(isset($vals->id)){
														...
														$uniq = pn_stryp_symbols(replace_cyr($bd_array['currency_get']));
														$uniq = unique_xml_value($uniq, 0);
