...
add_action('pn_direction_delete', 'pn_direction_delete_dopsumcomis');
function pn_direction_delete_dopsumcomis($item_id){
global $wpdb;	
	$wpdb->query("DELETE FROM ".$wpdb->prefix."naps_dopsumcomis WHERE naps_id = '$item_id'");
}

add_action('pn_direction_copy', 'pn_direction_copy_dopsumcomis', 1, 2);
function pn_direction_copy_dopsumcomis($last_id, $new_id){
global $wpdb;
...
function get_dopsumcomis_html_line($item){
	...
	$temp = '
	<div class="construct_line js_dopsumcomis_line" data-id="'. $item_id .'">
...
/* filter */
add_filter('get_calc_data', 'get_calc_data_dopsumcomis', 7, 2);
