...
global $wpdb, $premiumbox;
	...
	if($count_day > 0){
		...
		$items = $wpdb->get_results("SELECT * FROM ".$wpdb->prefix."exchange_bids WHERE create_date < '$ldate' LIMIT $limit");
			...
			if($result == 1){
				...
				if($status == 'success'){
					...
					if($pcalc == 1){
						...
						set_archive_data($item->ref_id, 'pbids_sum', '', '', $partner_sum);
				...
				$arr['ref_id'] = is_isset($item,'ref_id');
				...
				$arr['psys_give'] = is_isset($item,'psys_give');
				$arr['psys_get'] = is_isset($item,'psys_get');
				...
				$arr['course_give'] = is_isset($item,'course_give');
				$arr['course_get'] = is_isset($item,'course_get');
				$arr['user_ip'] = is_isset($item,'user_ip');
				$arr['currency_code_give'] = is_isset($item,'currency_code_give');
				$arr['currency_code_get'] = is_isset($item,'currency_code_get');
				...
				$arr['user_discount'] = is_isset($item,'user_discount');
				$arr['user_discount_sum'] = is_isset($item,'user_discount_sum');
				...
				$arr['pay_ac'] = is_isset($item,'pay_ac');
				$arr['pay_sum'] = is_isset($item,'pay_sum');
				$arr['sum1'] = is_isset($item,'sum1');
				$arr['dop_com1'] = is_isset($item,'dop_com1');
				$arr['sum1dc'] = is_isset($item,'sum1dc');
				$arr['com_ps1'] = is_isset($item,'com_ps1');
				$arr['com_ps2'] = is_isset($item,'com_ps2');
				$arr['sum1c'] = is_isset($item,'sum1c');
				$arr['sum1r'] = is_isset($item,'sum1r');
				$arr['sum2t'] = is_isset($item,'sum2t');
				$arr['sum2'] = is_isset($item,'sum2');
				$arr['dop_com2'] = is_isset($item,'dop_com2');
				$arr['sum2dc'] = is_isset($item,'sum2dc');
				$arr['sum2r'] = is_isset($item,'sum2r');
				$arr['sum2c'] = is_isset($item,'sum2c');
				$wpdb->insert($wpdb->prefix."archive_exchange_bids", $arr);
					
				do_action('change_bidstatus_all', 'archived', $item->id, $item, 'archive', 'system');
				do_action('change_bidstatus_archived', $item->id, $item, 'archive', 'system');
