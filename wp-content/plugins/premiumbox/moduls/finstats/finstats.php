		...
		<div class="fin_list">
			...
			<select name="currency_code_id" autocomplete="off">
				...
				<?php foreach($currency_codes as $item){ ?>
					<option value="<?php echo $item->id; ?>"><?php echo is_site_value($item->currency_code_title); ?></option>
				<?php } ?>
			</select>
...
global $wpdb;
	...
	if(current_user_can('administrator') or current_user_can('pn_finstats')){
		...
		if($startdate){
			...
			$where1 .= " AND create_date >= '$startdate'";
			$where2 .= " AND create_date >= '$startdate'";
		...
		if($enddate){
			...
			$where1 .= " AND create_date <= '$enddate'";
			$where2 .= " AND create_date <= '$enddate'";
		...
		if($currency_id){
			$data = $wpdb->get_row("SELECT * FROM ". $wpdb->prefix ."currency WHERE auto_status='1' AND id='$currency_id'");
			if(isset($data->id)){
		...
		if($currency_code_id){
			$data = $wpdb->get_row("SELECT * FROM ". $wpdb->prefix ."currency_codes WHERE auto_status='1' AND id='$currency_code_id'");
			if(isset($data->id)){			
		...
		$bou_c = $wpdb->get_var("SELECT SUM(sum1dc) FROM ".$wpdb->prefix."exchange_bids WHERE status IN('payed','realpay','verify','coldsuccess','success') $where1");
		$bou = $bou + $bou_c;
		...
		$sol_c = $wpdb->get_var("SELECT SUM(sum2c) FROM ".$wpdb->prefix."exchange_bids WHERE status = 'success' $where2");
		$sol = $sol + $sol_c;
		...
		if($ppay == 1){
			$query = $wpdb->query("CHECK TABLE ".$wpdb->prefix ."user_payouts");
			if($query == 1){
				$partn = $wpdb->get_var("SELECT SUM(pay_sum) FROM ".$wpdb->prefix."user_payouts WHERE status = '1' $where3");
				$bou = $bou - $partn;
				$ac_bou = $ac_bou - $partn;
			}
		}
		...
		if($convert){
			$data = $wpdb->get_row("SELECT * FROM ". $wpdb->prefix ."currency_codes WHERE auto_status = '1' AND id='$convert'");
			if(isset($data->id)){
